...
import { authenticate } from '@/helper/webauthn/login'
...
export const WebAuthnSignInForm = () => {
  ...
  async function onSubmit(values: z.infer<typeof webAuthnSignInFormSchema>) {
    ...
    const getChallenge = await fetch('/api/webauthn/getChallenge/login', {
      method: "POST",
      headers: {
    ...
    const { challenge_id, challenge, credIds } = await getChallenge.json();
    const authentication = await authenticate(credIds, challenge, {
